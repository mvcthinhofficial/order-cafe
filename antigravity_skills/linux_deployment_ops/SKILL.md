...
✅ routes/                  ← BẮT BUỘC: server.cjs require('./routes/paymentWebhook.cjs')
...
**Nguyên nhân cụ thể (04/2026):**

`routes/paymentWebhook.cjs` bị thiếu trong tar.gz build trên GitHub CI:

```bash
# CI cũ (SAI) — thiếu routes/
tar -czf bundle.tar.gz dist server.cjs db.cjs src/utils public ...

# Kết quả trên server khi start:
# Error: Cannot find module './routes/paymentWebhook.cjs'
# → server.cjs crash ngay dòng require cuối file
# → PM2 ↺ restart loop 144 lần rồi bỏ cuộc
```

**Fix đã áp dụng:**
1. Thêm `routes/` vào `PACK_FILES` trong `deploy_linux.sh`
2. Thêm `routes/` vào GitHub CI workflow `release.yml` với existence check
3. Thêm pre-flight validation check trước khi `tar`
