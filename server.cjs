...
// XÓA VĨNH VIỄN phiếu nhập (chỉ áp dụng cho phiếu đã soft-delete)
app.delete('/api/imports/:id/permanent', (req, res) => {
    const { id } = req.params;
    const authHeader = req.headers.authorization;
    const user = activeTokens.get((authHeader || '').replace('Bearer ', ''));
    if (!user || user.role !== 'ADMIN') {
        return res.status(403).json({ success: false, message: 'Chỉ Admin mới có thể xóa vĩnh viễn' });
    }

    try {
        // Chỉ xóa phiếu đã được đánh dấu isDeleted
        const result = db.prepare('DELETE FROM imports WHERE id = ? AND isDeleted = 1').run(id);
        if (result.changes === 0) {
            return res.status(404).json({ success: false, message: 'Không tìm thấy phiếu đã xóa để xóa vĩnh viễn' });
        }
        // Sync in-memory
        imports = imports.filter(imp => imp.id !== id);
        console.log(`[IMPORTS] Đã xóa vĩnh viễn phiếu nhập ID: ${id}`);
        res.json({ success: true });
    } catch (err) {
        console.error('Error permanently deleting import:', err);
        res.status(500).json({ success: false, message: 'Lỗi khi xóa vĩnh viễn' });
    }
});
    ...
    setTimeout(() => {
        ...
        curlProcess.on('close', (curlCode) => {
            ...
            tarProcess.on('close', (tarCode) => {
                ...
                // [Fix] Do Linux server không có đủ môi trường build C++ (gcc, python), 
                // lệnh npm install sẽ lỗi. Cần KHÔI PHỤC rule: KHÔNG chạy npm install
                // nếu better-sqlite3 đã tồn tại.
                ...
                if (stillMissing) {
                    ...
                    console.log('[SystemUpdate] node_modules đã có sẵn (giữ nguyên binary native đang chạy ổn định).');
                    doRestart();
                }
