...
app.post('/api/system/update', (req, res) => {
    ...
    // Command sequence: download, extract, update deps, restart
    const cmd = `curl -L -o update_pkg.tar.gz "${downloadUrl}" && tar -xzf update_pkg.tar.gz && npm install --omit=dev && (pm2 restart order-cafe || pm2 start server.cjs --name order-cafe)`;

    console.log(`[UPDATE] Starting update from: ${downloadUrl}`);
    res.json({ success: true, message: 'Đang bắt đầu quá trình cập nhật trong nền. Server sẽ tự động khởi động lại sau ít phút.' });
...
app.post('/api/order', (req, res) => {
    ...
    // Luôn ưu tiên dùng nextQueueNumber của server
    ...
    if (order) {
        ...
        if (order.cartItems && order.cartItems.length > 0) {
            order.cartItems.forEach(cartItem => {
                // Find full item from menu to get the recipe (cartItem might be a snapshot)
                const menuItem = menu.find(m => m.id === cartItem.item.id);
                // Get multiplier for the selected size
                ...
                if (cartItem.size) {
                    ...
                    if (menuSize && menuSize.multiplier) {
                        sizeMultiplier = parseFloat(menuSize.multiplier);
                    }
                }
                ...
                if (menuItem && menuItem.recipe && Array.isArray(menuItem.recipe)) {
                    menuItem.recipe.forEach(recipeItem => {
                        ...
                        if (ingredient) {
                            const unitUsedQty = Math.ceil(recipeItem.quantity * sizeMultiplier);
                            const usedQty = unitUsedQty * cartItem.count;
                            ...
                            // Force usageHistory to be an object if it's an array or null
                            if (!ingredient.usageHistory || Array.isArray(ingredient.usageHistory)) {
                                ingredient.usageHistory = {};
                            }
            ...
            const todayStr = getVNDateStr();
                            ...
                            console.log(`[INVENTORY] Deducted ${usedQty} ${ingredient.unit} from ${ingredient.name} (Recipe: ${menuItem.name}, Mult: ${sizeMultiplier})`);
                        } else if (recipeItem.ingredientId) {
                            console.warn(`[INVENTORY] WARNING: Ingredient ID "${recipeItem.ingredientId}" not found for item "${menuItem.name}"`);
                        }
                    });
                }

                // Deduct from Size-specific recipes
                if (cartItem.size) {
                    const selectedSizeLabel = typeof cartItem.size === 'string' ? cartItem.size : (cartItem.size.label || cartItem.size.name);
                    const menuSize = menuItem?.sizes?.find(s => s.label === selectedSizeLabel);
                    if (menuSize && menuSize.recipe && Array.isArray(menuSize.recipe)) {
                        menuSize.recipe.forEach(recipeItem => {
                            const ingredient = inventory.find(inv => inv.id === recipeItem.ingredientId);
                            if (ingredient) {
                                const unitUsedQty = Math.ceil(recipeItem.quantity);
                                const usedQty = unitUsedQty * cartItem.count;
                                ingredient.stock = parseFloat((ingredient.stock - usedQty).toFixed(3));
                                if (!orderDeductions[ingredient.id]) orderDeductions[ingredient.id] = 0;
                                orderDeductions[ingredient.id] += usedQty;
                                ...
                                const todayStr = getVNDateStr();
                                ingredient.usageHistory[todayStr] = parseFloat(((ingredient.usageHistory[todayStr] || 0) + usedQty).toFixed(3));
                                console.log(`[INVENTORY] Deducted ${usedQty} ${ingredient.unit} from ${ingredient.name} (Size: ${selectedSizeLabel})`);
                            } else if (recipeItem.ingredientId) {
                                console.warn(`[INVENTORY] WARNING: Ingredient ID "${recipeItem.ingredientId}" not found for size "${selectedSizeLabel}"`);
                            }
                        });
                    }
                }
                ...
                if (cartItem.addons && Array.isArray(cartItem.addons)) {
                    cartItem.addons.forEach(addonItem => {
                        ...
                        if (menuAddon && menuAddon.recipe && Array.isArray(menuAddon.recipe)) {
                            menuAddon.recipe.forEach(recipeItem => {
                                const ingredient = inventory.find(inv => inv.id === recipeItem.ingredientId);
                                if (ingredient) {
                                    const unitUsedQty = Math.ceil(recipeItem.quantity);
                                    const usedQty = unitUsedQty * cartItem.count;
                ...
                const todayStr = getVNDateStr();
                                    ingredient.usageHistory[todayStr] = parseFloat(((ingredient.usageHistory[todayStr] || 0) + usedQty).toFixed(3));
                                    console.log(`[INVENTORY] Deducted ${usedQty} ${ingredient.unit} from ${ingredient.name} (Addon: ${addonLabel})`);
                                } else if (recipeItem.ingredientId) {
                                    console.warn(`[INVENTORY] WARNING: Ingredient ID "${recipeItem.ingredientId}" not found for addon "${addonLabel}"`);
                                }
                            });
                        }
                    });
        ...
        if (Object.keys(orderDeductions).length > 0) {
            ...
            inventory_audits.push({
                ...
                orderId: order.id,
                queueNumber: order.queueNumber,
                userName: (req.user && req.user.userName) ? req.user.userName : 'Hệ thống',
                inputs: auditInputs
            });
        }
