...
const Menu = ({ settings }) => {
    ...
    const addToCartFromModal = () => {
        ...
        setSelectedItem(null);
        // Removed automatic navigation - User stays on menu to add more
    };
                ...
                {selectedItem && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                        ...
                        <motion.div
                            ...
                            className="relative bg-white w-[92%] max-w-sm p-8 pb-12 pt-10 shadow-2xl z-10 border border-gray-100 rounded-none"
                        ...
                        >
                            <div className="absolute top-3 left-1/2 -translate-x-1/2 w-10 h-1.5 rounded-none bg-gray-200" />
                            {/* Modal Content - Reorganized Hierarchy */}
                            <div className="flex flex-col items-center">
                                {/* 1. Circular Image at the top */}
                                <div className="w-48 h-48 rounded-none overflow-hidden border-4 border-gray-50 shadow-lg mb-8 bg-gray-50 flex items-center justify-center">
                                    <img src={getImageUrl(selectedItem.image)} className="w-full h-full object-cover" alt="" />
                                </div>

                                {/* 2. Item Name & Details */}
                                <div className="text-center mb-10 w-full">
                                    <span className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em] mb-2 block">{selectedItem.category}</span>
                                    <h2 className="text-2xl font-black text-gray-900 leading-tight mb-2 uppercase tracking-tighter">{selectedItem.name}</h2>
                                    <p className="text-brand-500 font-black text-2xl">{formatVND(selectedItem.price)}</p>
                                </div>
                            </div>

                            <div className="p-6 max-h-[45vh] overflow-y-auto no-scrollbar" onPointerDownCapture={(e) => e.stopPropagation()}>
                                {/* Size selection */}
                                {selectedItem.sizes?.length > 0 && (
                                    <section className="mb-8">
                                        <div className="flex items-center gap-3 mb-4 px-1">
                                            <h4 className="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em]">● KÍCH CỠ</h4>
                                            <span className="text-[10px] font-bold text-accent bg-accent/5 px-2 py-0.5 rounded-none">Bắt buộc</span>
                                        </div>
                                        <div className="grid grid-cols-3 gap-3">
                                            {selectedItem.sizes.map(s => (
                                                <button key={s.label} onClick={() => setSelectedSize(s)}
                                                    className={`py-5 px-2 border-2 rounded-none transition-all flex flex-col items-center gap-1 active:scale-95 ${selectedSize?.label === s.label ? 'border-brand-500 bg-brand-50 text-brand-500' : 'border-gray-100 bg-white text-gray-400 hover:bg-gray-50'}`}
                                                >
                                                    <span className="font-black text-sm uppercase tracking-wider">{s.label}</span>
                                                    {s.priceAdjust > 0 && <span className="text-[11px] font-bold">+{formatVND(s.priceAdjust)}</span>}
                                                </button>
                                            ))}
                                        </div>
                                    </section>
                                ...
                                <div className="grid grid-cols-1 gap-8">
                                    {(() => {
                                        ...
                                        return (
                                            <section>
                                                <h4 className="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4 px-1">● MỨC ĐƯỜNG</h4>
                                                <div className="flex gap-3 overflow-x-auto no-scrollbar pb-2">
                                                    {opts.map(val => (
                                                        <button key={val} onClick={() => setSelectedSugar(val)}
                                                            className={`min-w-[80px] shrink-0 px-5 py-4 border-2 rounded-none active:scale-95 font-black text-sm transition-all ${selectedSugar === val ? 'border-brand-500 bg-brand-50 text-brand-500' : 'border-gray-100 bg-white text-gray-400 hover:bg-gray-50'}`}
                                                        >
                                                            {val}
                                                        </button>
                                                    ))}
                                    ...
                                    {(() => {
                                        ...
                                        return (
                                            <section>
                                                <h4 className="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4 px-1">● MỨC ĐÁ</h4>
                                                <div className="flex gap-3 overflow-x-auto no-scrollbar pb-2">
                                                    {opts.map(val => (
                                                        <button key={val} onClick={() => setSelectedIce(val)}
                                                            className={`min-w-[110px] shrink-0 px-5 py-4 border-2 font-black text-sm transition-all ${selectedIce === val ? 'border-brand-500 bg-brand-50 text-brand-500' : 'border-gray-100 bg-white text-gray-400 hover:bg-gray-50'}`}
                                                        >
                                                            {val}
                                                        </button>
                                                    ))}
                                ...
                                {selectedItem.addons?.length > 0 && (
                                    <section className="mt-8">
                                        <h4 className="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4 px-1">● TÙY CHỌN THÊM</h4>
                                        <div className="grid grid-cols-1 gap-3">
                                            {selectedItem.addons.map(addon => {
                                                const sel = selectedAddons.find(a => a.label === addon.label);
                                                return (
                                                    <button key={addon.label} onClick={() => toggleAddon(addon)}
                                                        className={`w-full px-5 py-4 border-2 transition-all flex justify-between items-center ${sel ? 'border-brand-500 bg-brand-50 text-brand-500' : 'border-gray-100 bg-white text-gray-400 hover:bg-gray-50'}`}
                                                    >
                                                        <div className="flex items-center gap-3">
                                                            <div className={`w-5 h-5 border flex items-center justify-center transition-colors ${sel ? 'bg-white border-brand-500' : 'border-gray-200'}`}>
                                                                {sel && <div className="w-2.5 h-2.5 bg-brand-500" />}
                                                            </div>
                                                            <span className={`font-bold text-sm ${sel ? 'text-brand-500' : 'text-gray-700'}`}>{addon.label}</span>
                                                        </div>
                                                        {addon.price > 0 && <span className="font-black text-sm">+{formatVND(addon.price)}</span>}
                                                    </button>
                                                );
                                            })}
                                        </div>
                            ...
                            <div className="pt-6 border-t border-gray-100 flex items-center justify-between gap-4">
                                <div className="flex flex-col">
                                    <span className="text-[10px] font-black text-gray-300 uppercase tracking-widest mb-1">TẠM TÍNH</span>
                                ...
                                <button
                                    ...
                                    className="flex-1 bg-accent text-white py-4 font-black text-sm shadow-xl active:scale-[0.98] transition-all flex items-center justify-center gap-2"
                                    
                                >
                                    <Plus size={18} strokeWidth={3} /> CHỌN MÓN NÀY
                                </button>
                            </div>
                        </motion.div>
                    </div>
