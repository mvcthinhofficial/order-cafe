...
const CustomerDrinkingCalendar = ({ customerId, customer }) => {
    ...
    return (
        ...
        >
            ...
            {/* ── Heatmap giờ cao điểm toàn quán ── */}
            <div style={{
                background: '#fff',
                border: '1px solid #E2E8F0',
                borderRadius: 'var(--radius-card)',
                ...
                <p style={{
                    ...
                    🔥 Giờ Cao Điểm Toàn Quán
                    <span style={{ fontSize: 10, color: '#9CA3AF', fontWeight: 600, textTransform: 'none', letterSpacing: 0 }}>
                        (tuần này · tất cả khách)
                    </span>
                </p>
                ...
                ) : Object.keys(hourTotal).length === 0 ? (
                    <p style={{ fontSize: 12, color: '#9CA3AF', textAlign: 'center', padding: '8px 0', margin: 0 }}>
                        Chưa có đơn nào trong tuần này
                    </p>
                ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                        {HOURS.map((hour) => {
                            const count = hourTotal[hour] || 0;
                            const c = getHeatColor(count, maxHourTotal);
                            const barPct = maxHourTotal > 0 ? Math.max(4, (count / maxHourTotal) * 100) : 4;
                            return (
                                <div key={hour} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                        ...
                                        <div style={{
                                            ...
                                            {count > 0 && (
                                                <span style={{
                                                    ...
                                                    color: count / maxHourTotal > 0.4 ? c.text : '#374151',
                                                    whiteSpace: 'nowrap',
                                                }}>
                                                    {count} đơn
                                                </span>
                                    ...
                                    {/* Chỉ báo "giờ vàng" */}
                                    {count > 0 && count === Math.max(...Object.values(hourTotal)) && (
                                        <span style={{
            ...
            {/* ── Gợi ý phân ca ── */}
            {!heatLoading && Object.keys(hourTotal).length > 0 && (() => {
                const peakHour = Object.entries(hourTotal).sort((a, b) => b[1] - a[1])[0];
                    ...
                    <div style={{
                        ...
                        <div>
                            <p style={{ margin: 0, fontSize: 12, fontWeight: 800, color: '#92400E' }}>
                                Gợi ý phân ca: Tăng cường nhân viên lúc {h}:00 – {parseInt(h, 10) + 1}:00
                            </p>
                            <p style={{ margin: '3px 0 0', fontSize: 11, color: '#B45309' }}>
                                Khung giờ này có {cnt} đơn — cao nhất trong tuần. Nên bố trí thêm 1–2 nhân viên.
                            </p>
                        </div>
                    </div>
                );
