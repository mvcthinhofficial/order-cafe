...
const TableActionModal = ({ table, onClose, onOrder, onUpdateStatus, onChangeTable }) => {
    ...
    return (
        <div className="fixed inset-0 z-[600] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 20, opacity: 0 }} className="bg-white w-full max-w-sm overflow-hidden shadow-2xl relative z-10 p-2">
                <div className="text-center border-b border-gray-50 bg-gray-50/50" style={{ padding: '32px' }}>
                ...
                <div className="p-6 space-y-3">
                    {table.activeOrder ? (
                        <div className="text-left border border-orange-200 overflow-hidden shadow-sm bg-white">
                            <div className="bg-orange-50 border-b border-orange-200 flex justify-between items-center" style={{ paddingLeft: '16px', paddingRight: '16px', paddingTop: '8px', paddingBottom: '8px' }}>
                            ...
                            <div className="p-4 space-y-3 max-h-[40vh] overflow-y-auto custom-scrollbar">
                                {(table.activeOrder.cartItems || []).map((c, idx) => (
                                    <div key={idx} className="flex flex-col text-sm border-b border-dashed border-gray-100 last:border-0 last:pb-0" style={{ paddingBottom: '12px' }}>
                                        ...
                                        <div className="flex flex-wrap gap-1 mt-1 pl-6">
                                            <span className="text-[10px] bg-gray-100 px-1.5 py-0.5 font-bold text-gray-600">S: {c.size?.label || 'S'}</span>
                                            {c.sugar && <span className="text-[10px] bg-amber-50 px-1.5 py-0.5 font-bold text-amber-700">Đường: {c.sugar}</span>}
                                            {c.ice && <span className="text-[10px] bg-brand-50 px-1.5 py-0.5 font-bold text-brand-700">Đá: {c.ice}</span>}
                                        </div>
                            ...
                            <div className="bg-gray-50 border-t border-gray-100 flex justify-between items-center" style={{ paddingLeft: '16px', paddingRight: '16px', paddingTop: '12px', paddingBottom: '12px' }}>
                        ...
                        <button onClick={() => onOrder()} className="w-full flex items-center justify-between p-6 bg-brand-600 text-white font-black text-base shadow-xl shadow-brand-500/20 active:scale-95 transition-all uppercase tracking-widest">
                    ...
                    <div className="grid grid-cols-2 gap-5">
                        {table.activeOrder ? (
                            <>
                                <button onClick={() => onChangeTable(table.activeOrder)} className="p-5 bg-brand-50 text-brand-600 font-black text-sm uppercase tracking-widest hover:bg-brand-100 active:scale-95 transition-all flex flex-col items-center justify-center gap-2">
                                    <ArrowRightLeft size={24} /> ĐỔI BÀN
                                </button>
                                <button onClick={() => onUpdateStatus('Available')} className="p-5 bg-gray-50 text-gray-400 font-black text-sm uppercase tracking-widest hover:bg-gray-100 hover:text-gray-500 active:scale-95 transition-all flex flex-col items-center justify-center gap-2">
                                    <CheckCircle size={24} /> TRỐNG
                                </button>
                            </>
                        ) : (
                            <>
                                <button onClick={() => onUpdateStatus('Available')} className="p-5 bg-gray-50 text-gray-500 font-black text-sm uppercase tracking-widest hover:bg-gray-100 active:scale-95 transition-all flex flex-col items-center justify-center gap-2">
                                    <CheckCircle size={24} /> TRỐNG
                                </button>
                                <button onClick={() => onUpdateStatus('Reserved')} className="p-5 bg-brand-50 text-brand-600 font-black text-sm uppercase tracking-widest hover:bg-brand-100 active:scale-95 transition-all flex flex-col items-center justify-center gap-2">
