                    ...
                    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[1100]">
                        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white max-w-sm w-full shadow-2xl overflow-hidden shadow-red-500/20">
                            ...
                            <div className="flex gap-3 border-t border-slate-100" style={{ padding: '24px' }}>
                                <button onClick={() => setDeleteMenuModal(null)} className="flex-1 px-4 py-4 bg-slate-100 text-slate-600 font-bold hover:bg-slate-200 transition-all active:scale-95 text-sm uppercase tracking-widest">Hủy</button>
                                <button onClick={confirmDeleteMenuItem} className="flex-1 px-4 py-4 bg-red-500 text-white font-bold hover:bg-red-600 transition-all active:scale-95 shadow-lg shadow-red-500/20 text-sm uppercase tracking-widest">Đồng Ý</button>
                            </div>
                        </motion.div>
                    </div>
                )}
