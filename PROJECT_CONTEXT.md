...
### 9.29 Loyalty Voucher ↔ Promotions Integration (04/04/2026)

#### Kiến trúc chuỗi liên kết (Chain of Value)

```
Admin tạo voucher trong CustomersTab
  → POST /api/loyalty/admin/customers/:id/send-voucher
  → INSERT vào promotions table (type=PROMO_CODE, code=TRIXXX, specificPhone, singleUse=true, ignoreGlobalDisable=true)
  → reloadPromotions() reload in-memory array ngay lập tức
  
Khách vào quán → nhân viên chọn khách trong StaffOrderPanel
  → loyaltyProfile.phone được set
  → hasActivePromoCode phát hiện promo có specificPhone === phone
  → Banner vàng "🎁 Khách có voucher TRI-XXXX — Tap để điền"
  → Nhân viên tap → auto-fill code → engine verify → giảm giá áp dụng
  → Sau thanh toán: singleUse voucher mark isActive=false (DB + in-memory)

Khách kiosk/mobile nhập SĐT
  → customerProfile.phone set từ loyalty API
  → hasActivePromoCode detect specificPhone === phone
  → Banner "🎁 Bạn có voucher cá nhân!" trong giỏ hàng
  → Auto-expand promo input + pre-fill code khi tap
```

#### Schema Voucher (lưu trong field `data` JSON của bảng promotions)

```javascript
{
  id: 'promo_voucher_TIMESTAMP',
  name: 'Voucher TRIXXX',
  description: 'Tri ân — Dành riêng cho Chị Lan',
  type: 'PROMO_CODE',         // BẮT BUỘC để engine nhận diện
  code: 'TRIXXX',             // Mã khách nhập — uppercase 8 ký tự
  discountType: 'PERCENT',    // PERCENT | FIXED
  discountValue: 20,          // Số %
  minOrderValue: 0,
  applicableItems: ['ALL'],
  isActive: true,
  singleUse: true,            // Vô hiệu hoá sau khi dùng 1 lần
  specificPhone: '0979...',   // NULL nếu voucher global
  specificCustomerId: 'cust_...',
  startDate: '2026-04-04',
  endDate: '2026-04-18',
  ignoreGlobalDisable: true,  // Voucher cá nhân không bị tắt bởi globalDisable
}
```

#### PromotionEngine — tham số mới

```javascript
// Thêm currentCustomerPhone là param cuối (optional, default null)
calculateCartWithPromotions(cart, promotions, promoCodeInput, menu, selectedPromoId, enablePromotions, currentCustomerPhone)

// Logic filter PROMO_CODE với specificPhone:
// - Nếu specificPhone && currentCustomerPhone khớp → áp dụng được
// - Nếu specificPhone && không có currentCustomerPhone → từ chối (bảo vệ)
// - Nếu không có specificPhone → voucher global ai cũng dùng được
```
...
| File | Thay đổi |
|---|---|
| `server/routes/loyaltyRoutes.cjs` | Fix `send-voucher` lưu full promo object; nhận `reloadPromotions` callback |
| `server.cjs` | Inject `reloadPromotions` vào loyaltyRoutes; deactivate `singleUse` voucher sau order |
| `src/utils/promotionEngine.js` | Thêm `currentCustomerPhone` param; filter specificPhone |
| `StaffOrderPanel.jsx` | Detect voucher cá nhân, hint banner vàng tap-to-fill |
| `CustomerKiosk.jsx` | Detect voucher cá nhân theo `customerProfile.phone` |
| `BillView.jsx` | Detect voucher cá nhân theo `loyaltyProfile.phone` |
| `AdminDashboard.jsx` | Pass `promotions` prop vào `<CustomersTab>` |
| `CustomersTab.jsx` | Nhận `promotions` prop, hiển thị voucher đang sống, cho phép hủy voucher |

#### Quy tắc bất biến khi thêm tính năng voucher mới

> ⚠️ **Không bao giờ INSERT voucher vào DB mà không có `type: 'PROMO_CODE'` và `code` đầy đủ trong JSON `data`** — engine sẽ không nhận ra và voucher sẽ "chết" ngay khi tạo.

> ⚠️ **`reloadPromotions` phải được gọi ngay sau INSERT** — in-memory promotions array không tự cập nhật khi DB thay đổi từ route khác.

> ⚠️ **Voucher `singleUse` được deactivate khi ORDER TẠO** (không phải khi thanh toán) để tránh giữ mã quá lâu trên đơn nợ.

---

*Cập nhật lần cuối: 04/04/2026 — Tích hợp Loyalty Voucher với Promotions system. Voucher cá nhân liên kết từ CustomersTab → engine → UI giỏ hàng mọi giao diện.*
